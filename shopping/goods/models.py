...
class Category(models.Model):   #商品分类
    name = models.CharField("商品分类", max_length=64)
...
def upload_to_goods_img(instance,filename):
    return 'goods/{prefix}_{filename}.{suffix}'.format(prefix='goods',filename=int(time.time()*1000),suffix=filename.split('.')[-1])
...
class Goods(models.Model):  #商品信息
    ...
    price = models.FloatField("商品价格", default=0)
    img = models.ImageField("商品图片",upload_to=upload_to_goods_img)
    create_time = models.DateField("商品添加时间")
    store = models.IntegerField("商品剩余库存", default=0)
    desc = models.TextField("商品详情描述")
    status = models.IntegerField("状态", default=1)
...
class ShoppingCar(models.Model):    #购物车
...
class GoodsBuied(models.Model): #历史订单
